...
namespace DoozyEngine.Utils
{
    public class RotatedRectangle
    {
...
#if DEBUG
            try {
                this.texture = new Texture2D(RootEngine.Graphics, 1, 1);
        ...
        private bool IsAxisCollision(RotatedRectangle theRectangle, Vector2 aAxis) {
            ...
            if (aRectangleBMinimum <= aRectangleAMaximum && aRectangleBMaximum >= aRectangleAMinimum) {
                return true;
            }
        ...
        private Vector2 RotatePoint(Vector2 thePoint, Vector2 theOrigin, float theRotation) {
            ...
            aTranslatedPoint.X = (float)(this.Origin.X + (Math.Cos(theRotation) * (this.Origin.X - thePoint.X)));
            aTranslatedPoint.Y = (float)(this.Origin.Y + (Math.Sin(theRotation) * (this.Origin.Y - thePoint.Y)));
            aTranslatedPoint.X = (float)(this.Origin.X + (thePoint.X - this.Origin.X) * Math.Cos(theRotation) - (thePoint.Y - this.Origin.Y) * Math.Sin(theRotation));
            aTranslatedPoint.Y = (float)(this.Origin.Y + (thePoint.Y - this.Origin.Y) * Math.Cos(theRotation) + (thePoint.X - this.Origin.X) * Math.Sin(theRotation));
        ...
        public int X { get { return CollisionRectangle.X; } }
        public int Y { get { return CollisionRectangle.Y; } }

        public int Width { get { return CollisionRectangle.Width; } }
        public int Height { get { return CollisionRectangle.Height; } }


#if DEBUG
        public void Draw() {
            if (!this.IsEnable || this.texture == null)
                return;

            

            DisplayManager.SpriteBatch.Draw(this.texture,
                this.CollisionRectangle, //width of line, change this to make thicker line
                null,
                Color.Purple * 0.5f, //colour of line
                this.Rotation,     //angle of line (calulated above)
                new Vector2(this.Origin.X - this.X, this.Origin.Y - this.Y), // point in line about which to rotate
                SpriteEffects.None,
                1f);

            DisplayManager.SpriteBatch.Draw(this.texture,
                new Rectangle((int)this.UpperLeftCorner().X,
                    (int)this.UpperLeftCorner().Y,
                    10,10),//width of line, change this to make thicker line
                null,
            ...
            DisplayManager.SpriteBatch.Draw(this.texture,
                new Rectangle((int)this.UpperRightCorner().X,
                    (int)this.UpperRightCorner().Y,
                    10, 10),//width of line, change this to make thicker line
                null,
            ...
            DisplayManager.SpriteBatch.Draw(this.texture,
                new Rectangle((int)this.LowerLeftCorner().X,
                    (int)this.LowerLeftCorner().Y,
                    10, 10),//width of line, change this to make thicker line
                null,
            ...
            DisplayManager.SpriteBatch.Draw(this.texture, new Rectangle((int)this.LowerRightCorner().X, (int)this.LowerRightCorner().Y, 10, 10), null, Color.Yellow, 0, Vector2.Zero, SpriteEffects.None, 1f);

            DisplayManager.SpriteBatch.Draw(this.texture, new Rectangle((int)this.Origin.X, (int)this.Origin.Y, 10, 10), null, Color.Black, 0, Vector2.Zero, SpriteEffects.None, 1f);
        }
#endif
    }
}
